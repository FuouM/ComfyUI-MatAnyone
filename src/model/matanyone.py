...
class MatAnyone(nn.Module):
    def __init__(self, model_cfg, *, single_object=False):
        ...
        self.aux_computer = AuxComputer(model_cfg)
    ...
    def load_weights(self, src_dict, init_as_zero_if_needed=False) -> None:
        if not self.single_object:
            ...
            for k in list(src_dict.keys()):
                if k == "mask_encoder.conv1.weight":
                    if src_dict[k].shape[1] == 4:
                        print(f"Converting {k} from single object to multiple objects.")
                        pads = torch.zeros((64, 1, 7, 7), device=src_dict[k].device)
                        if not init_as_zero_if_needed:
                            nn.init.orthogonal_(pads)
                            print(f"Randomly initialized padding for {k}.")
                        else:
                            print(f"Zero-initialized padding for {k}.")
            ...
            if src_dict["mask_encoder.conv1.weight"].shape[1] == 5:
                print(
                    "WARNING: Converting mask_encoder.conv1.weight from multiple objects to single object."
        ...
        for k in src_dict:
            if k not in self.state_dict():
                print(f"Key {k} found in src_dict but not in self.state_dict()!!!")
        for k in self.state_dict():
            if k not in src_dict:
                print(f"Key {k} found in self.state_dict() but not in src_dict!!!")
